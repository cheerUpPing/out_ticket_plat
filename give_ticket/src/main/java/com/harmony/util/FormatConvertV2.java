 ...
 */
public class FormatConvertV2 {
    private final static Logger __logger = Logger.getLogger(FormatConvertV2.class);
    ...
    private static final String TRADITIONNEW_REG =
            ...
            "第\\s*(\\d+)期[\\s*\\S*]*追加投注[\\s*]?(\\d+)倍\\s*合计(\\d+)元";//传统足球&数字彩期号倍数金额正则
    ...
    private static final String ELEVENCHOOSEFIVE_REGTWO =
            "第(\\d+)期.*[\\s*\\S]*票(\\d+)倍\\s*合计(\\d+)元";//乌鲁木齐11选5期号倍数金额正则2
...

    {
...

        {
            ...
            data = ticketInfo.replace("\u001BE", "").replace("P怂\u001B3", "").replace("\u001B3", "").replace("\u001B", "").replace("\u0001", "")
                    ...
                    .replace("#", "").replaceAll("\\x00", "").replace("a", "").replace("�", "").replace("湧", "").replace("璹", "").replace("ゆ", "").replace("顐", "").replace("庢", "")
...
//						String reg = "元(.*)";
                        ...
                        } else if (data.indexOf("直选和值票") != -1)

                        {

                            chipin.setManner("31");

                            String reg = "元\\s*([\\d{1,2}\\s]+)";
...
//						String reg = "元(.*)";
                        ...
                        } else if (data.indexOf("直选组合胆拖票") != -1)
...
//				}
			...
			/*if (data.indexOf("<全国联网排列5>") != -1 || data.indexOf("体彩<排列5>") != -1)*/
//			if ((data1.indexOf("·排列5") != -1||data1.indexOf("<排列5>") != -1)&&gamecode==109)
            if (gamecode == 109) {
...

                {
                    ...
                    {
//						String reg1 = "位:\\s*(\\d{1}|\\s*)(\\d{1}|\\s*)(\\d{1}|\\s*)(\\d{1}|\\s*)(\\d{1}|\\s*)(\\d{1}|\\s*)(\\d{1}|\\s*)(\\d{1}|\\s*)(\\d{1}|\\s*)(\\d{1}|\\s*)";
                        chipin.setManner("2");
                        data = data.replace(".", "");
                        String reg = "位:\\s*([\\d|\\s*]+)";

//						data =data.replace(".","");
//						String reg = "位:\\s*(\\d{10})\\s*";
            ...
            if (ticketInfo.indexOf("上下盘单双数") != -1)
...
//			if ((data1.indexOf("<11选5>") != -1||data1.indexOf("·11选5") != -1)&&(gamecode==405||gamecode==401||gamecode==402||gamecode==404))
            if (gamecode == 405 || gamecode == 401 || gamecode == 402 || gamecode == 404 || gamecode == 405 || gamecode == 406 || gamecode == 407 || gamecode == 408 || gamecode == 409) {
...
//						}
                        ...
                        if (data.indexOf("复式票") != -1 || data.indexOf("单式票") != -1)
                    ...
                    if (data.indexOf("选前2直选") != -1 || data.indexOf("选前二直选") != -1)
                    ...
                    if (data.indexOf("选前2组选") != -1 || data.indexOf("选前二组选") != -1)
                    ...
                    if (data.indexOf("选前3直选") != -1 || data.indexOf("选前三直选") != -1)
...
//			list = new ArrayList<String>();
...

            {
                ...
                String[] strings = temp.replace("元", "").replaceAll("\\\n", "").split(" ");
